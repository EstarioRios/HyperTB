...
from .optimation import donwload_status_checker, remove_from_now_down_list
import time
    ...
    while True:
        ...
        async def youtube_download(event, link, resolution, size_mb):
            try:
                ...
                error, video_path = await download_youtube_video(
                    link, resolution, user_id
                )
                if not error:
                    ...
                    remove_from_now_down_list(
                        item_down_link=link, item_resolution=resolution, user_id=user_id
                    )
        ...
        async def soundcloud_download(event):
            ...
            if not error:
                ...
                if os.path.exists(file_path):
                    os.remove(file_path)
                remove_from_now_down_list(
                    item_down_link=link, item_resolution=None, user_id=event.sender_id
                )
